...
def find_projective_correspondence(source_points,
    ...
    mask = ((target_us < w) & (target_us >= 0) & (target_vs >= 0) & (target_vs < h) & (target_ds >= 0))
    # End of TODO
    ...
    mask = np.zeros_like(target_us).astype(bool)
    target_q = target_vertex_map[target_vs, target_us]
    dists = np.linalg.norm(T_source_points - target_q, axis=1)
    mask = dists < dist_diff
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    A[:, 0] = p_prime[:, 1] * n_q[:, 2] - p_prime[:, 2] * n_q[:, 1]
    A[:, 1] = p_prime[:, 2] * n_q[:, 0] - p_prime[:, 0] * n_q[:, 2]
    A[:, 2] = p_prime[:, 0] * n_q[:, 1] - p_prime[:, 1] * n_q[:, 0]
    A[:, 3:] = n_q
    b = np.sum((p_prime - q) * n_q, axis=1)
    # End of TODO
...
def solve(A, b):
    ...
    # The question is formulated as minimization of Ax + b. However, the optimizer solves the minimization of Ax - b.
