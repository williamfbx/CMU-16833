...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        self.points[indices] = (self.weights[indices] * self.points[indices] + 1 * points_w) / (self.weights[indices] + 1)
        self.normals[indices] = (self.weights[indices] * self.normals[indices] + 1 * normals_w) / (self.weights[indices] + 1)
        self.normals[indices] /= np.linalg.norm(self.normals[indices], axis=1, keepdims=True)
        self.colors[indices] = (self.weights[indices] * self.colors[indices] + 1 * colors) / (self.weights[indices] + 1)
    ...
    def add(self, points, normals, colors, R, t):
        ...
        points_w = (R @ points.T + t).T
        normals_w = (R @ normals.T).T
        normals_w /= np.linalg.norm(normals_w, axis=1, keepdims=True)
        new_weights = np.ones((points.shape[0], 1))
        new_ages = np.zeros((points.shape[0], 1))

        self.points = np.concatenate((self.points, points_w), axis=0)
        self.normals = np.concatenate((self.normals, normals_w), axis=0)
        self.colors = np.concatenate((self.colors, colors), axis=0)
        self.weights = np.concatenate((self.weights, new_weights), axis=0)
        self.age = np.concatenate((self.age, new_ages), axis=0)
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        mask_dist = dist < dist_diff
        
        # Angles from dot product: https://www.cuemath.com/geometry/angle-between-vectors/      
    ...
    def fuse(self,
        ...
        if not self.initialized:
            ...
            # Remove dormant points
            max_age = 20
            min_weight = 15
            
            flat_weights = self.weights.flatten()
            active_mask = (self.age[:, 0] < max_age) | (flat_weights >= min_weight)

            self.points = self.points[active_mask]
            self.normals = self.normals[active_mask]
            self.colors = self.colors[active_mask]
            self.weights = self.weights[active_mask]
            self.age = self.age[active_mask]
